...
async function findDeckByDeckId(deckId) {
  ...
  try {
    ...
    if (rec) {
      ...
      // ✅ FIX: si el deck existe en Decks pero no trae cartas, leer desde Cards por deck_id
      if (!cards || cards.length === 0) {
        try {
          cards = await fetchCardsByDeckId(fields.deck_id || deckId);
        } catch (e) {
          // si falla, seguimos con cards vacío
        }
      }
...
async function getDeckForProduct(productId) {
  ...
  if (Array.isArray(deckId) && deckId.length === 1) {
    ...
    try {
      ...
      // ✅ también aquí: si viene vacío desde Decks record, intentar por deck_id
      if (!cards || cards.length === 0) {
        try {
          cards = await fetchCardsByDeckId(fields.deck_id || f.deck_id || ''); // best effort
        } catch (e) {}
      }
