...
module.exports = (req, res) => {
  try {
    const filePath = path.join(process.cwd(), 'data', 'decks', 'angeles.json');
    if (!fs.existsSync(filePath)) {
      return res.status(500).json({ ok: false, error: 'Deck file not found on server' });
    }
    ...
    return res.status(200).json({
      ok: true,
      spread: 'angeles_4',
      deck: { slug: deck.slug || 'angeles', name: deck.name },
      seed: seed || null,
