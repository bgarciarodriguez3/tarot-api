...
// ==============================
// GLOBAL ERROR LOGS (CLAVE PARA 502)
// ==============================
...
function normalizeFormPayload(body = {}) {
  ...
  const orderId =
    body.orderId ||
    getValueFromAnswers(answers, ["pedido", "order id"])
  ...
  if (payload.orderId && payload.email && payload.premiumType) {
    const r = await pool.query(
      `SELECT * FROM premium_requests
       WHERE order_id=$1 AND email=$2 AND premium_type=$3
...
function buildPremiumButton(url) {
  ...
  <table align="center" style="margin:30px auto;">
    <tr>
      <td style="border-radius:999px;background:linear-gradient(135deg,#7b5cff,#c6a45a);padding:16px 28px;">
        <a href="${url}" style="color:#fff;font-weight:bold;text-decoration:none;">
...
app.post("/api/premium/form-submitted", async (req, res) => {
  try {
    ...
    console.log("MATCH:", requestRecord?.id)
